package com.example.mad_practical_7_22012011142
...
DatabaseHelper(context:Context?):SQLiteOpenHelper(
    ...
    fun insertPerson(
        name: String, emailId: String, phoneNo: String, address:
        String
    ): Long {
    ...
    fun getAllPersons(): List<Person> {
        ...
        with(cursor) {
            while (moveToNext()) {
                val person = Person(
                    id =
                    getString(getColumnIndexOrThrow(PersonDbTableData.COLUMN_ID)),
                    name =
                    getString(getColumnIndexOrThrow(PersonDbTableData.COLUMN_PERSON_NAME)),
                    emailId =
                    getString(getColumnIndexOrThrow(PersonDbTableData.COLUMN_PERSON_EMAIL_ID)),
                    phoneNo =
                    getString(getColumnIndexOrThrow(PersonDbTableData.COLUMN_PERSON_PHONE_NO)),
                    address =
                    getString(getColumnIndexOrThrow(PersonDbTableData.COLUMN_PERSON_ADDRESS)),

                    latitude =
                    getDoubleOrNull(getColumnIndexOrThrow(PersonDbTableData.COLUMN_PERSON_GPS_LAT)),
                    longitude =
                    getDoubleOrNull(
                        getColumnIndexOrThrow(PersonDbTableData.COLUMN_PERSON_GPS_LONG)
                    )
                )
